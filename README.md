# Philipp â€” a self-hosted personal dashboard

A suite of nine single-file apps tied together by one bento **hub** (`index.html`),
one design system ("Patron" â€” soft-dark electric-violet), and one shared theme.
Fork it, deploy it (or just open it), make it yours.

### ðŸš€ Get your own copy live in one click

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/ohwisey/patron)

Click the button â†’ it copies this repo to your GitHub and deploys it automatically.
No fork, no setup â€” you get a live `your-app.vercel.app` link in about a minute.
(Optional API keys for fitness-band & creator sync: see **`SETUP.md`**.)

| App | File | What it does |
|---|---|---|
| **Hub** | `index.html` | Bento launcher Â· onboarding Â· "Today's vitals" Â· live stats |
...
1. Fork this repo.
2. Import it into [Vercel](https://vercel.com) (no build step â€” it's static + `/api` functions).
3. (Optional) set the env vars below, then open your deployed URL.

## How your data is stored

Everything lives in **your browser's `localStorage`** â€” there is no central database
...
profile), `patron_health_v1` (vitals â€” Whoop / Apple Watch / manual).

## Optional integrations

| Feature | Where | How |
|---|---|---|
| **Whoop** live recovery/sleep/strain | Hub vitals + `whoop.html` | Server-side OAuth â€” see [`WHOOP_SETUP.md`](WHOOP_SETUP.md). Needs Vercel + env vars. |
| **Apple Watch** | Hub vitals | No web API exists â€” enter your latest Health readings manually. |
| **Manual vitals** | Hub vitals | Type recovery + sleep; feeds the supplement recommender. |
| **Stock prices** (Finance) | Finance âš™ settings | Paste a free [Finnhub](https://finnhub.io) key (stored in your browser). |
| **AI statement import** (Finance) | Finance âš™ settings | Paste an [Anthropic](https://console.anthropic.com) key (Haiku, downscaled images â€” cheap). |
| **Gym sync** | Gym â†’ âš™ Settings â†’ Cloud sync | Optional â€” paste *your own* Supabase URL + publishable key (else local-only). See below. |
...
### Gym cloud sync â€” use your own Supabase (optional)
...
2. **Project â†’ Settings â†’ API:** copy the **Project URL** + the **publishable** key (`sb_publishable_â€¦`).
3. **Project â†’ SQL Editor:** run this once:
   ```sql
...
4. In the app: **Gym â†’ âš™ Settings â†’ Cloud sync**, paste the URL + key, **Save & sync**.
...
MIT â€” see [`LICENSE`](LICENSE). Fork it, deploy it, make it yours.

## Disclaimer

This is a personal-tracking tool, **not** medical, dietary, or financial advice.
Supplement doses and recommendations are general information â€” consult a professional.
